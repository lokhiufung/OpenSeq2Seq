...
class Speech2Text(EncoderDecoderModel):
  ...
  def maybe_print_logs(self, input_values, output_values, training_step):
    ...
    sample_cer = levenshtein(split_text_to_chars(true_text), split_text_to_chars(pred_text)) / \
        len(split_text_to_chars(true_text))
  ...
  def evaluate(self, input_values, output_values):
    ...
    for sample_id in range(batch_size):
      ...
      total_char_lev += levenshtein(split_text_to_chars(true_text), split_text_to_chars(pred_text))
      total_char_count += len(split_text_to_chars(true_text))
